 ...
 * Cambia el estado operativo de una habitación según una acción solicitada.
 * Reglas de negocio:
 * - Solo staff autorizado.
 * - Se valida la acción y el contexto de reservas antes de mutar estado.
 */
export async function POST(
  ...
  try {
    ...
    const { id: roomId } = await params; // Agregado await params
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // VALIDACIONES MEJORADAS por acción
      switch (action) {
        case 'OPEN':
        ...
        case 'CLOSED':
          // NO se puede cerrar si hay huésped dentro
          if (hasCheckedIn) {
            throw new Error(
          ...
          // Advertir si hay reservas futuras
          if (hasActiveReservations) {
        ...
        case 'MAINTENANCE':
          // NO se puede poner en mantenimiento si hay huésped
          if (hasCheckedIn) {
            throw new Error(
          ...
          // Advertir si hay reservas próximas
        ...
        case 'CLEANING':
        case 'DIRTY':
          // NO se puede marcar para limpieza si está cerrada
          if (room.status === 'CLOSED') {
            throw new Error(
          ...
          // NO se puede si hay huésped dentro
          if (hasCheckedIn) {
            throw new Error(
        ...
        case 'CLEAN':
          // Solo desde CLEANING se puede marcar como limpia
