...
export default function ServicePaymentPage({
  ...
  const [paymentData, setPaymentData] = useState<ServicePaymentData | null>(null);
  ...
  const fetchPaymentData = async () => {
    try {
      const response = await fetch(`/api/payments/services/${paymentId}`);
  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await fetch('/api/payments/services/create-preference', {
        method: 'POST',
        headers: {
  ...
  const handleTestPayment = async () => {
    ...
    try {
      const response = await fetch('/api/test-payment-service', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        router.push(`/services/${paymentData?.booking.id}`);
      } else {
  ...
  const bookingCode = generateServiceBookingCode(paymentData.booking.id);
  const imageUrl = paymentData.booking.service.mainImage || paymentData.booking.service.images[0] || null;
      ...
      <main className="min-h-screen bg-[#f3eee7]">
        ...
        <div className="mx-auto max-w-3xl px-6 pt-24 pb-12">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
            ...
            <Link
              href={`/services/${paymentData.booking.id}`}
              className="inline-block bg-black text-white px-6 py-2 rounded-lg hover:bg-neutral-800 transition"
            >
              Ver reserva de servicio
            </Link>
          </div>
        </div>
        <Footer />
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 h-full flex flex-col">
              ...
              <div className="flex gap-4 mb-6">
                {imageUrl && (
                  <div className="w-24 h-24 bg-neutral-200 rounded-lg overflow-hidden flex-shrink-0">
                    <img
                      src={imageUrl}
                      alt={paymentData.booking.service.name}
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <span className="inline-block px-2 py-0.5 bg-neutral-100 text-neutral-700 text-xs rounded">
                      {getServiceTypeLabel(paymentData.booking.service.type)}
                    </span>
                    <span className="inline-block px-2 py-0.5 bg-neutral-100 text-neutral-700 text-xs rounded">
                      {getServiceCategoryLabel(paymentData.booking.service.category)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <p className="font-medium text-black">
                    {formatDate(paymentData.booking.bookingDate)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-neutral-600 mb-1">Horario</p>
                  <p className="font-medium text-black">
                    {formatServiceTime(paymentData.booking.bookingTime, paymentData.booking.service.duration)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-neutral-600 mb-1">Duración</p>
                  <p className="font-medium text-black">
                    {formatDuration(paymentData.booking.service.duration)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-neutral-600 mb-1">Participantes</p>
                  <p className="font-medium text-black">
                    {paymentData.booking.participants}{' '}
                    {paymentData.booking.participants === 1 ? 'persona' : 'personas'}
                  </p>
                </div>
              </div>
              ...
              <div className="mb-4 p-3 bg-neutral-50 border border-neutral-200 rounded-lg">
                ...
                <p className="text-sm text-neutral-700">
                  Habitación #{paymentData.booking.reservation.room.number} · {' '}
                  {formatShortDate(paymentData.booking.reservation.checkIn)} - {' '}
                  {formatShortDate(paymentData.booking.reservation.checkOut)}
                </p>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 h-full flex flex-col">
              ...
              <div className="space-y-2 mb-4">
                <div className="flex justify-between text-sm">
                  <span className="text-neutral-600">
                    Precio base{' '}
                    {paymentData.booking.service.pricePerPerson && 
                      `x ${paymentData.booking.participants}`
                    }
                  </span>
                  <span className="text-neutral-900">
                    {formatPrice(
                      paymentData.booking.service.pricePerPerson
                        ? paymentData.booking.service.price * paymentData.booking.participants
                        : paymentData.booking.service.price
                    )}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <div className="mt-auto space-y-3">
                ...
                <button
                  ...
                  disabled={processing}
                  className="w-full bg-green-100 border border-green-200 text-green-700 hover:bg-green-200 text-center font-medium py-2 px-4 rounded-[30px] transition text-sm disabled:opacity-50"
                >
                  Pagar reserva
                </button>
